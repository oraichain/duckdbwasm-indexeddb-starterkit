...
(async () => {
  ...
  const defaultTasks = await conn.query('select * from todos');

  const saveDuckdb = async () => {
    // Export Parquet
    await conn.send(`copy (select * from todos) to 'todos.parquet'`);
    await set('todos', await db.copyFileToBuffer('todos.parquet'));
  };

  const App = () => {
    const [tasks, setTasks] = useState(defaultTasks.toArray());
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <div className={`hero ${darkTheme ? 'bg-gray-900' : 'bg-gray-100'} h-screen md:min-h-[700px]  w-full m-auto flex flex-col items-center mt-14 transition-all duration-500`}>
          <div className={`flex flex-col space-y-6 w-[600px] md:w-[100%] z-10 p-4 ${darkTheme ? 'text-white' : 'text-black'}`}>
            ...
            <div className={`scroll ${darkTheme ? 'bg-gray-800' : 'bg-white'} w-full h-[400px] md:h-[500px] px-2 overflow-y-scroll rounded-md shadow-lg relative transition-all duration-500`}>
